...
use spacetimedb_client_api::auth::{validate_token, SpacetimeAuth, SpacetimeCreds};
...
use spacetimedb_lib::{AlgebraicType, AlgebraicValue, ProductTypeElement, ProductValue};
use thiserror::Error;
...
fn type_of(ty: &ProductTypeElement) -> Type {
    ...
    for row in stmt.rows {
        ...
        for idx in 0..ncols {
            ...
            match value {
                ...
                AlgebraicValue::U8(x) => encoder.encode_field(&(*x as i32)).unwrap(),
                ...
                AlgebraicValue::String(x) => encoder.encode_field(&x.to_string()).unwrap(),
        ...
        match message {
            ...
            PgWireFrontendMessage::PasswordMessageFamily(pwd) => {
                ...
                match response(name.resolve(&self.ctx).await, &database).await {
                    ...
                    PgResult::Fail(res) => {
                        let err = pgwire::messages::response::ErrorResponse::from(*res);
                ...
                let claims = match validate_token(&self.ctx, &pwd.password).await {
                    Ok(claims) => claims,
                    Err(err) => {
                        let error_info = ErrorInfo::new("FATAL".to_owned(), "28P01".to_owned(), err.to_string());
                ...
                let mut auth = SpacetimeAuth::alloc(&self.ctx).await.unwrap();
                auth.identity = claims.identity;
                auth.creds = SpacetimeCreds::from_signed_token(pwd.password);
...
fn setup_tls(xcert: CertificateAuthority) -> Result<TlsAcceptor, io::Error> {
    use rcgen::{CertificateParams, DistinguishedName, DnType, KeyPair};

    let keys = xcert.get_or_create_keys2().unwrap();
    let private = keys.private_key_bytes;
...
pub async fn start_pg(mut shutdown: watch::Receiver<()>, ctx: Arc<StandaloneEnv>, certs: CertificateAuthority) {
    ...
    let temp = SpacetimeAuth::alloc(&ctx).await.unwrap();
    ...
    let tls_acceptor = Arc::new(setup_tls(certs).unwrap());
